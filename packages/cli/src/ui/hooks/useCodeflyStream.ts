...
import type {
  ...
  ServerCodeflyContentEvent as ContentEvent,
  ServerCodeflyFinishedEvent,
  ServerCodeflyStreamEvent as CodeflyEvent,
  ThoughtSummary,
  ToolCallRequestInfo,
  ToolCallResponseInfo,
  CodeflyErrorEventValue,
  RetryAttemptPayload,
} from '@codeflyai/codefly-core';
...
function calculateStreamingState(
  ...
  const isAnyToolActive = toolCalls.some((tc) => {
    ...
    // if the result hasn't been submitted back to Codefly yet.
    if (
...
/**
 * Manages the Codefly stream, including user input, command processing,
 * API interaction, and tool call lifecycle.
 */
export const useCodeflyStream = (
  ...
  const [lastCodeflyActivityTime, setLastCodeflyActivityTime] =
    useState<number>(0);
  ...
  const prepareQueryForCodefly = useCallback(
    async (
      query: PartListUnion,
      ...
      let localQueryToSendToCodefly: PartListUnion | null = null;
      ...
      if (typeof query === 'string') {
        ...
        if (!shellModeActive) {
          ...
          if (slashCommandResult) {
            switch (slashCommandResult.type) {
              ...
              case 'submit_prompt': {
                localQueryToSendToCodefly = slashCommandResult.content;

                return {
                  queryToSend: localQueryToSendToCodefly,
                  shouldProceed: true,
                };
              }
              case 'handled': {
        ...
        if (isAtCommand(trimmedQuery)) {
          ...
          localQueryToSendToCodefly = atCommandResult.processedQuery;
        } else {
          // Normal query for Codefly
          addItem(
            { type: MessageType.USER, text: trimmedQuery },
            userMessageTimestamp,
          );
          localQueryToSendToCodefly = trimmedQuery;
        }
      } else {
        // It's a function response (PartListUnion that isn't a string)
        localQueryToSendToCodefly = query;
      }

      if (localQueryToSendToCodefly === null) {
        onDebugMessage(
          'Query processing resulted in null, not sending to Codefly.',
        );
        return { queryToSend: null, shouldProceed: false };
      }
      return { queryToSend: localQueryToSendToCodefly, shouldProceed: true };
    },
    [
      config,
      addItem,
  ...
  const handleContentEvent = useCallback(
    (
      ...
      currentCodeflyMessageBuffer: string,
      userMessageTimestamp: number,
    ): string => {
      setRetryStatus(null);
      if (turnCancelledRef.current) {
      ...
      let newCodeflyMessageBuffer = currentCodeflyMessageBuffer + eventValue;
      if (
        pendingHistoryItemRef.current?.type !== 'codefly' &&
        pendingHistoryItemRef.current?.type !== 'codefly_content'
      ) {
        // Flush any pending item before starting codefly content
        if (pendingHistoryItemRef.current) {
          addItem(pendingHistoryItemRef.current, userMessageTimestamp);
        }
        setPendingHistoryItem({ type: 'codefly', text: '' });
        newCodeflyMessageBuffer = eventValue;
      }
      // Split large messages for better rendering performance. Ideally,
      // we should maximize the amount of output sent to <Static />.
      const splitPoint = findLastSafeSplitPoint(newCodeflyMessageBuffer);
      if (splitPoint === newCodeflyMessageBuffer.length) {
        // Update the existing message with accumulated content
        setPendingHistoryItem((item) => ({
          // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
          type: item?.type as 'codefly' | 'codefly_content',
          text: newCodeflyMessageBuffer,
        }));
      } else {
        // This indicates that we need to split up this Codefly Message.
        ...
        const beforeText = newCodeflyMessageBuffer.substring(0, splitPoint);
        const afterText = newCodeflyMessageBuffer.substring(splitPoint);
        ...
        setPendingHistoryItem({ type: 'codefly_content', text: afterText });
        newCodeflyMessageBuffer = afterText;
      }
      return newCodeflyMessageBuffer;
    },
  ...
  const handleErrorEvent = useCallback(
    (eventValue: CodeflyErrorEventValue, userMessageTimestamp: number) => {
      if (pendingHistoryItemRef.current) {
        addItem(pendingHistoryItemRef.current, userMessageTimestamp);
        setPendingHistoryItem(null);
      }
  ...
  const handleFinishedEvent = useCallback(
    (event: ServerCodeflyFinishedEvent, userMessageTimestamp: number) => {
  ...
  const processCodeflyStreamEvents = useCallback(
    async (
      stream: AsyncIterable<CodeflyEvent>,
      userMessageTimestamp: number,
      signal: AbortSignal,
    ): Promise<StreamProcessingStatus> => {
      let codeflyMessageBuffer = '';
      const toolCallRequests: ToolCallRequestInfo[] = [];
      for await (const event of stream) {
        if (
          event.type !== ServerCodeflyEventType.Thought &&
        ...
        switch (event.type) {
          case ServerCodeflyEventType.Thought:
            setLastCodeflyActivityTime(Date.now());
            handleThoughtEvent(event.value, userMessageTimestamp);
            break;
          case ServerCodeflyEventType.Content:
            setLastCodeflyActivityTime(Date.now());
            codeflyMessageBuffer = handleContentEvent(
              event.value,
              codeflyMessageBuffer,
              userMessageTimestamp,
            );
            break;
  ...
  const submitQuery = useCallback(
      ...
      runInDevTraceSpan(
        ...
        async ({ metadata: spanMetadata }) => {
          ...
          return promptIdContext.run(prompt_id, async () => {
            const { queryToSend, shouldProceed } = await prepareQueryForCodefly(
              query,
              userMessageTimestamp,
              abortSignal,
              prompt_id!,
            ...
            try {
              const stream = codeflyClient.sendMessageStream(
                queryToSend,
                abortSignal,
                prompt_id!,
                undefined,
              ...
              const processingStatus = await processCodeflyStreamEvents(
                stream,
                userMessageTimestamp,
                abortSignal,
              );
    ...
    [
      ...
      prepareQueryForCodefly,
      processCodeflyStreamEvents,
      pendingHistoryItemRef,
      addItem,
      setPendingHistoryItem,
      setInitError,
  ...
  const handleApprovalModeChange = useCallback(
    ...
    [config, toolCalls, codeflyClient, streamingState, addItem, onDebugMessage],
  );
  ...
  const handleCompletedTools = useCallback(
    async (completedToolCallsFromScheduler: TrackedToolCall[]) => {
      ...
      const codeflyTools = completedAndReadyToSubmitTools.filter(
        (t) => !t.request.isClientInitiated,
      );

      if (codeflyTools.length === 0) {
        return;
      }

      // Check if any tool requested to stop execution immediately
      const stopExecutionTool = codeflyTools.find(
      ...
      if (stopExecutionTool && stopExecutionTool.response.error) {
        ...
        const callIdsToMarkAsSubmitted = codeflyTools.map(
      ...
      // If all the tools were cancelled, don't submit a response to Codefly.
      const allToolsCancelled = codeflyTools.every(
      ...
      if (allToolsCancelled) {
        ...
        if (codeflyClient) {
          ...
          const combinedParts = codeflyTools.flatMap(
      ...
      const responsesToSend: Part[] = codeflyTools.flatMap(
        (toolCall) => toolCall.response.responseParts,
      );
      ...
      const prompt_ids = codeflyTools.map(
