...
import { useCodeflyStream } from './useCodeflyStream.js';
...
const MockedCodeflyClientClass = vi.hoisted(() =>
  vi.fn().mockImplementation(function (this: any, _config: any) {
    ...
    this.getCurrentSequenceModel = vi
      .fn()
      .mockReturnValue('codefly-2.0-flash-exp');
  }),
);
...
// --- Tests for useCodeflyStream Hook ---
describe('useCodeflyStream', () => {
  ...
  const mockConfig: Config = {
    ...
    getModel: vi.fn(() => 'codefly-2.5-pro'),
  ...
  const renderTestHook = (
    ...
    const client = codeflyClient || mockConfig.getCodeflyClient();
          ...
          lastToolCalls = lastToolCalls.map((tc) => {
            if (
              ...
              return {
                ...
                responseSubmittedToCodefly: true,
  ...
  describe('Slash Command Handling', () => {
    ...
    it('should stop processing and not call Codefly when a command is handled without a tool call', async () => {
    ...
    it('should call Codefly with prompt content when slash command returns a `submit_prompt` action', async () => {
  ...
  it('should flush pending text rationale before scheduling tool calls to ensure correct history order', async () => {
    ...
    // Expectation: addItem:codefly (rationale) MUST happen before scheduleToolCalls_START
    const rationaleIndex = addItemOrder.indexOf('addItem:codefly');
  ...
  it('should display user query, then tool execution, then model response', async () => {
    ...
    // Mock the Codefly stream to return a model response after the tool
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield {
          type: ServerCodeflyEventType.Content,
  ...
  describe('Thought Reset', () => {
    ...
    it('should update lastOutputTime on Codefly thought and content events', async () => {
      vi.useFakeTimers();
  ...
  describe('Stream Splitting', () => {
    it('should not add empty history item when splitting message results in empty or whitespace-only beforeText', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield { type: ServerCodeflyEventType.Content, value: 'test content' };
        })(),
      );
    ...
    it('should add whitespace-only history item when splitting message', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield { type: ServerCodeflyEventType.Content, value: '   content' };
        })(),
      );
