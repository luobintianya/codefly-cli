...
describe('initCommand', () => {
  ...
  it('should inform the user if CODEFLY.md already exists', async () => {
    ...
    expect(result).toEqual({
      ...
      messageType: 'info',
      content:
        'A CODEFLY.md file already exists in this directory. No changes were made.',
    });
    // Assert: Ensure no file was written
    expect(fs.writeFileSync).not.toHaveBeenCalled();
  });

  it('should create CODEFLY.md and submit a prompt if it does not exist', async () => {
    ...
    expect(mockContext.ui.addItem).toHaveBeenCalledWith(
      {
        type: 'info',
        text: 'Empty CODEFLY.md created. Now analyzing the project to populate it.',
      },
      expect.any(Number),
    );
