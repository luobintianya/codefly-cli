  ...
  try {
    if (config.command === 'sandbox-exec') {
      ...
      if (!BUILTIN_SEATBELT_PROFILES.includes(profile)) {
        profileFile = path.join(CODEFLY_DIR, `sandbox-macos-${profile}.sb`);
      }
    ...
    const userSettingsDirInSandbox = getContainerPath(
      `/home/node/${CODEFLY_DIR}`,
    );
    if (!fs.existsSync(userHomeDirOnHost)) {
      fs.mkdirSync(userHomeDirOnHost, { recursive: true });
    }
    const userSettingsDirOnHost = path.join(userHomeDirOnHost, CODEFLY_DIR);
    ...
    if (
      ...
      const sandboxVenvPath = path.resolve(CODEFLY_DIR, 'sandbox.venv');
