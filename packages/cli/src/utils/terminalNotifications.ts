...
import { debugLogger, writeToStdout } from '@codeflyai/codefly-core';
...
function sanitizeNotificationContent(
  ...
  return {
    title: title || 'Codefly CLI',
    subtitle: subtitle || undefined,
    body: body || 'Open Codefly CLI for details.',
  };
}
...
export function buildRunEventNotificationContent(
  ...
  if (event.type === 'attention') {
    return sanitizeNotificationContent({
      title: 'Codefly CLI needs your attention',
      subtitle: event.heading ?? 'Action required',
      body: event.detail ?? 'Open Codefly CLI to continue.',
    });
  }

  return sanitizeNotificationContent({
    title: 'Codefly CLI session complete',
