...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .codeflyignore patterns even when not in a git repo', async () => {
      await createTestFile('.codeflyignore', 'secrets.txt');
      const service = new FileDiscoveryService(projectRoot);
    ...
    it('should correctly resolve options passed to constructor', () => {
      ...
      expect(defaults.respectCodeflyIgnore).toBe(false);
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
      await createTestFile('.codeflyignore', 'logs/');
    });

    it('should filter out git-ignored and codefly-ignored files by default', () => {
      const files = [
        'src/index.ts',
    ...
    it('should not filter files when respectGitIgnore is false', () => {
      ...
      const filtered = service.filterFiles(files, {
        respectGitIgnore: false,
        respectCodeflyIgnore: true, // still respect this one
      });
    ...
    it('should not filter files when respectCodeflyIgnore is false', () => {
      const files = [
        'src/index.ts',
        'node_modules/package/index.js',
        'logs/latest.log',
  ...
  describe('filterFilesWithReport', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile('.codeflyignore', '*.log');
    });
  ...
  describe('shouldGitIgnoreFile & shouldCodeflyIgnoreFile', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile('.codeflyignore', '*.log');
    ...
    it('should return true for codefly-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);
    ...
    it('should return false for non-codefly-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);
  ...
  describe('precedence (.codeflyignore over .gitignore)', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
    });

    it('should un-ignore a file in .codeflyignore that is ignored in .gitignore', async () => {
      await createTestFile('.gitignore', '*.txt');
      await createTestFile('.codeflyignore', '!important.txt');
    ...
    it('should un-ignore a directory in .codeflyignore that is ignored in .gitignore', async () => {
    ...
    it('should extend ignore rules in .codeflyignore', async () => {
      await createTestFile('.gitignore', '*.log');
      await createTestFile('.codeflyignore', 'temp/');
    ...
    it('should use .gitignore rules if respectCodeflyIgnore is false', async () => {
      await createTestFile('.gitignore', '*.txt');
      await createTestFile('.codeflyignore', '!important.txt');
    ...
    it('should use .codeflyignore rules if respectGitIgnore is false', async () => {
      await createTestFile('.gitignore', '*.txt');
      await createTestFile('.codeflyignore', '!important.txt\ntemp/');
      ...
      // .codeflyignore un-ignores important.txt (which wasn't ignored anyway)
      // and ignores temp/
      expect(filtered).toEqual(
  ...
  describe('custom ignore file', () => {
    ...
    it('should prioritize custom ignore patterns over .codeflyignore patterns in git repo', async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile(CODEFLY_IGNORE_FILE_NAME, '*.log');

      const customIgnoreName = '.customignore';
      // .codeflyignore ignores *.log, custom un-ignores debug.log
    ...
    it('should prioritize custom ignore patterns over .codeflyignore patterns in non-git repo', async () => {
      // No .git directory created
      await createTestFile(CODEFLY_IGNORE_FILE_NAME, 'secret.txt');

      const customIgnoreName = '.customignore';
      // .codeflyignore ignores secret.txt, custom un-ignores it
  ...
  describe('getIgnoreFilePaths & getAllIgnoreFilePaths', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', '*.log');
      await createTestFile(CODEFLY_IGNORE_FILE_NAME, '*.tmp');
      await createTestFile('.customignore', '*.secret');
    });

    it('should return .codeflyignore path by default', () => {
      const service = new FileDiscoveryService(projectRoot);
      const paths = service.getIgnoreFilePaths();
      expect(paths).toEqual([path.join(projectRoot, CODEFLY_IGNORE_FILE_NAME)]);
    });

    it('should not return .codeflyignore path if respectCodeflyIgnore is false', () => {
      const service = new FileDiscoveryService(projectRoot, {
        respectCodeflyIgnore: false,
      });
      const paths = service.getIgnoreFilePaths();
    ...
    it('should return custom ignore file paths', () => {
      ...
      const paths = service.getIgnoreFilePaths();
      expect(paths).toContain(path.join(projectRoot, CODEFLY_IGNORE_FILE_NAME));
