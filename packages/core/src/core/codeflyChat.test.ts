...
describe('CodeflyChat', () => {
  ...
  beforeEach(() => {
    ...
    let currentModel = 'codefly-pro';
    let currentActiveModel = 'codefly-pro';
    ...
    mockConfig = {
      ...
      modelConfigService: {
        getResolvedConfig: vi.fn().mockImplementation((modelConfigKey) => {
          ...
          const thinkingConfig = model.startsWith('codefly-3')
            ? {
  ...
  describe('sendMessageStream', () => {
    ...
    it('should use thinkingLevel and remove thinkingBudget for codefly-3 models', async () => {
      const response = (async function* () {
        yield {
          candidates: [
            {
      ...
      const stream = await chat.sendMessageStream(
        { model: 'codefly-3-test-only-model-string-for-testing' },
        'hello',
    ...
    it('should use thinkingBudget and remove thinkingLevel for non-codefly-3 models', async () => {
      const response = (async function* () {
        yield {
          candidates: [
            {
  ...
  describe('sendMessageStream with retries', () => {
    it('should not retry on invalid content if model does not start with codefly-2', async () => {
    ...
    it('should pass attempted model to onPersistent429 callback which calls handleFallback', async () => {
      ...
      mockRetryWithBackoff.mockImplementation(async (apiCall, options) => {
        try {
          ...
          if (options?.onPersistent429) {
            await options.onPersistent429(AuthType.USE_CODEFLY, e);
          }
      ...
      // handleFallback is called with the ATTEMPTED model (model-a), not the requested one (codefly-pro)
      expect(mockHandleFallback).toHaveBeenCalledWith(
        expect.anything(),
        'model-a',
        expect.anything(),
