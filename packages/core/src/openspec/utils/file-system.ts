...
export class FileSystemUtils {
  ...
  static async fileExists(filePath: string): Promise<boolean> {
    try {
      ...
      if (error.code !== 'ENOENT') {
        console.debug(
          `Unable to check if file exists at ${filePath}: ${error.message}`,
        );
      }
      return false;
    }
  ...
  static async ensureWritePermissions(dirPath: string): Promise<boolean> {
    try {
      ...
      for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
          ...
          if (attempt === maxRetries - 1) {
            ...
            console.debug(
              `Could not clean up test file ${testFile}: ${unlinkError.message}`,
            );
          } else {
      ...
      console.debug(
        `Insufficient permissions to write to ${dirPath}: ${error.message}`,
      );
      return false;
    }
  }
