  ...
  generate(commands: CommandDefinition[]): string {
    ...
    for (const cmd of commands) {
      ...
      commandCaseLines.push(
        `          _openspec_${this.sanitizeFunctionName(cmd.name)}`,
      );
      commandCaseLines.push('          ;;');
    }
...
`;
  ...
  private generateCommandFunction(cmd: CommandDefinition): string[] {
    ...
    if (cmd.subcommands && cmd.subcommands.length > 0) {
      ...
      for (const subcmd of cmd.subcommands) {
        ...
        lines.push(
          `          _openspec_${this.sanitizeFunctionName(cmd.name)}_${this.sanitizeFunctionName(subcmd.name)}`,
        );
        lines.push('          ;;');
      }
  ...
  private generateFlagSpec(flag: FlagDefinition): string {
    ...
    if (flag.short) {
      parts.push(
        `'(-${flag.short} --${flag.name})'{-${flag.short},--${flag.name}}'`,
      );
    } else {
