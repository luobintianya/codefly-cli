...
`;
  ...
  private generatePositionalCompletion(
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(
          `${indent}Get-OpenSpecChanges | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {`,
        );
        lines.push(
      ...
      case 'spec-id':
        lines.push(
          `${indent}Get-OpenSpecSpecs | Where-Object { $_ -like "$wordToComplete*" } | ForEach-Object {`,
        );
        lines.push(
      ...
      case 'change-or-spec-id':
        lines.push(
          `${indent}$items = @(Get-OpenSpecChanges) + @(Get-OpenSpecSpecs)`,
        );
        lines.push(
