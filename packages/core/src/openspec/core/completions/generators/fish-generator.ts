...
${commandCompletions}`;
  ...
  private generatePositionalCompletion(
    ...
    switch (positionalType) {
      case 'change-id':
        lines.push(
          `complete -c openspec -n '${condition}' -a '(__fish_openspec_changes)' -f`,
        );
        break;
      case 'spec-id':
        lines.push(
          `complete -c openspec -n '${condition}' -a '(__fish_openspec_specs)' -f`,
        );
        break;
      case 'change-or-spec-id':
        lines.push(
          `complete -c openspec -n '${condition}' -a '(__fish_openspec_items)' -f`,
        );
        break;
      case 'shell':
        lines.push(
          `complete -c openspec -n '${condition}' -a 'zsh bash fish powershell' -f`,
        );
        break;
      case 'path':
