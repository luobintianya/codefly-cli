 ...
 */
export function validateConfigKeyPath(path: string): {
  valid: boolean;
  reason?: string;
} {
  ...
  if (rootKey === 'featureFlags') {
    if (rawKeys.length > 2) {
      return {
        valid: false,
        reason:
          'featureFlags values are booleans and do not support nested keys',
      };
    }
    return { valid: true };
  }
  ...
  if (rawKeys.length > 1) {
    return {
      valid: false,
      reason: `"${rootKey}" does not support nested keys`,
    };
  }
...
export function formatValueYaml(value: unknown, indent: number = 0): string {
  ...
  if (Array.isArray(value)) {
    ...
    return value
      .map((item) => `${indentStr}- ${formatValueYaml(item, indent + 1)}`)
      .join('\n');
  }
