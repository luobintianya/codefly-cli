...
export class ViewCommand {
  async execute(targetPath: string = '.'): Promise<void> {
      ...
      changesData.active.forEach((change) => {
        ...
        const percentage =
          change.progress.total > 0
            ? Math.round(
                (change.progress.completed / change.progress.total) * 100,
              )
            : 0;
    ...
    if (specsData.length > 0) {
      ...
      specsData.forEach((spec) => {
        const reqLabel =
          spec.requirementCount === 1 ? 'requirement' : 'requirements';
        console.log(
  ...
  private async getChangesData(openspecDir: string): Promise<{
    ...
    active.sort((a, b) => {
      const percentageA =
        a.progress.total > 0 ? a.progress.completed / a.progress.total : 0;
      const percentageB =
        b.progress.total > 0 ? b.progress.completed / b.progress.total : 0;
  ...
  private displaySummary(
    ...
    const totalRequirements = specsData.reduce(
      (sum, spec) => sum + spec.requirementCount,
      0,
    );
    ...
    if (changesData.draft.length > 0) {
      console.log(
        `  ${chalk.gray('●')} Draft Changes: ${chalk.bold(changesData.draft.length)}`,
      );
    }
    console.log(
