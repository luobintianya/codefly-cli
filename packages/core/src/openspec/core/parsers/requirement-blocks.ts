...
export function parseDeltaSpec(content: string): DeltaPlan {
  ...
  const modifiedLookup = getSectionCaseInsensitive(
    sections,
    'MODIFIED Requirements',
  );
  const removedLookup = getSectionCaseInsensitive(
    sections,
    'REMOVED Requirements',
  );
  const renamedLookup = getSectionCaseInsensitive(
    sections,
