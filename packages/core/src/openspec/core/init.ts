...
// Use the injected CLI version or fallback for dev/testing
const OPENSPEC_VERSION = process.env.CLI_VERSION || '0.0.0-dev';
...
export class InitCommand {
  ...
  private async handleLegacyCleanup(projectPath: string): Promise<void> {
    ...
    if (!canPrompt) {
      ...
      console.log(
        chalk.dim(
          'Run interactively to upgrade, or use --force to auto-cleanup.',
        ),
      );
      process.exit(1);
    }
  ...
  private displaySuccessMessage(
    ...
    if (successfulTools.length > 0) {
      ...
      if (hasCommands) {
        console.log(
          `${getSkillTemplates().length} skills and ${getCommandContents().length} commands in ${toolDirs}/`,
        );
      } else {
