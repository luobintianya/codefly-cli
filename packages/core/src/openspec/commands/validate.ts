...
export class ValidateCommand {
  ...
  private printReport(
    ...
    if (report.valid) {
      console.log(
        `${type === 'change' ? 'Change' : 'Specification'} '${id}' is valid`,
      );
    } else {
      console.error(
  ...
  private async runBulkValidation(
    ...
    if (opts.json) {
      ...
      console.log(
        `Totals: ${summary.totals.passed} passed, ${summary.totals.failed} failed (${summary.totals.items} items)`,
      );
    }
