...
export function registerSchemaCommand(program: Command): void {
    ...
    .action(
      async (name?: string, options?: { json?: boolean; all?: boolean }) => {
        try {
          ...
          if (!name) {
            console.error(
              'Error: Schema name is required (or use --all to list all schemas)',
            );
            process.exitCode = 1;
            return;
          }
        ...
        try {
          ...
          if (!isValidSchemaName(destinationName)) {
            if (options?.json) {
              ...
              console.error(
                'Schema names must be kebab-case (e.g., my-workflow)',
              );
            }
            process.exitCode = 1;
            return;
          ...
          if (fs.existsSync(destinationDir)) {
            if (!options?.force) {
              if (options?.json) {
                ...
                console.error(
                  `Error: Schema '${destinationName}' already exists at ${destinationDir}`,
                );
  ...
  schemaCmd
    ...
    .option(
      '--artifacts <list>',
      'Comma-separated artifact IDs (proposal,specs,design,tasks)',
    )
    ...
    .action(
      async (
        ...
        try {
          ...
          if (fs.existsSync(schemaDir)) {
            if (!options?.force) {
              if (options?.json) {
                  ...
                  JSON.stringify(
                    {
                      ...
                      suggestion:
                        'Use --force to overwrite or "openspec schema fork" to copy',
                    },
                    null,
                    2,
                  ),
                ...
                console.error(
                  `Error: Schema '${name}' already exists at ${schemaDir}`,
                );
                console.error(
                  'Use --force to overwrite or "openspec schema fork" to copy',
                );
          ...
          if (isInteractive) {
            // Interactive mode
            const { input, checkbox, confirm } = await import(
              '@inquirer/prompts'
            );
            ...
            description =
              options?.description || `Custom workflow schema for ${name}`;
          ...
          if (options?.json) {
            ...
            console.log(`\nNext steps:`);
            console.log(
              `  1. Edit ${schemaDir}/schema.yaml to customize artifacts`,
            );
