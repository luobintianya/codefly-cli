...
import type { CommandActionReturn, Config } from '@codeflyai/codefly-core';
  ...
  describe('execute', () => {
    it('handles info from performInit', async () => {
      vi.mocked(performInit).mockReturnValue({
        type: 'message',
        messageType: 'info',
        content: 'CODEFLY.md already exists.',
        ...
        expect.objectContaining({
          ...
          status: expect.objectContaining({
            ...
            message: expect.objectContaining({
              parts: [{ kind: 'text', text: 'CODEFLY.md already exists.' }],
            }),
          }),
        }),
      );
    ...
    describe('when handling submit_prompt', () => {
      beforeEach(() => {
        vi.mocked(performInit).mockReturnValue({
          type: 'submit_prompt',
          content: 'Create a new CODEFLY.md file.',
        } as CommandActionReturn);
      });
      ...
      it('writes the file and executes the agent', async () => {
        ...
        expect(fs.writeFileSync).toHaveBeenCalledWith(
          path.join(mockWorkspacePath, 'CODEFLY.md'),
          '',
          'utf8',
        );
          ...
          expect.objectContaining({
            userMessage: expect.objectContaining({
              parts: expect.arrayContaining([
                expect.objectContaining({
                  text: 'Create a new CODEFLY.md file.',
                }),
              ]),
              metadata: {
                coderAgent: {
