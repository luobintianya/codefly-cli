...
Codefly CLI can integrate with your IDE to provide a more seamless and
...
- **Native diffing:** When Codefly suggests code modifications, you can view the
...
- **VS Code commands:** You can access Codefly CLI features directly from the VS
  Code Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `Codefly CLI: Run`: Starts a new Codefly CLI session in the integrated
    terminal.
...
When you run Codefly CLI inside a supported editor, it will automatically detect
...
manually, you can run the following command inside Codefly CLI:
...
- **For Visual Studio Code:** Install from the
  [VS Code Marketplace](https://marketplace.visualstudio.com/items?itemName=google.codefly-cli-vscode-ide-companion).
- **For VS Code forks:** To support forks of VS Code, the extension is also
  published on the
  [Open VSX Registry](https://open-vsx.org/extension/google/codefly-cli-vscode-ide-companion).
  Follow your editor's instructions for installing extensions from this
  registry.

> NOTE: The "Codefly CLI Companion" extension may appear towards the bottom of
...
When enabled, Codefly CLI will automatically attempt to connect to the IDE
companion extension.
...
When you ask Codefly to modify a file, it can open a diff view directly in your
editor.
...
- Open the Command Palette and run **Codefly CLI: Accept Diff**.
...
- Open the Command Palette and run **Codefly CLI: Close Diff Editor**.
...
If you are using Codefly CLI within a sandbox, please be aware of the following:
...
- **On macOS:** The IDE integration requires network access to communicate with
  ...
  network access.
- **In a Docker container:** If you run Codefly CLI inside a Docker (or Podman)
...
If automatic IDE detection fails, or if you are running Codefly CLI in a
standalone terminal and want to manually associate it with a specific IDE
instance, you can set the `CODEFLY_CLI_IDE_PID` environment variable to the
process ID (PID) of your IDE.

```bash
export CODEFLY_CLI_IDE_PID=12345
```

When this variable is set, Codefly CLI will skip automatic detection and attempt
to connect using the provided PID.

### Configuration errors

- **Message:**
  `🔴 Disconnected: Directory mismatch. Codefly CLI is running in a different location than the open workspace in [IDE Name]. Please run the CLI from one of the following directories: [List of directories]`
...
- **Message:**
  `IDE integration is not supported in your current environment. To use this feature, run Codefly CLI in one of these supported IDEs: [List of IDEs]`
  - **Cause:** You are running Codefly CLI in a terminal or environment that is
    not a supported IDE.
  - **Solution:** Run Codefly CLI from the integrated terminal of a supported
    IDE, like Antigravity or VS Code.

- **Message:**
  `No installer is available for IDE. Please install the Codefly CLI Companion extension manually from the marketplace.`
  - **Cause:** You ran `/ide install`, but the CLI does not have an automated
    installer for your specific IDE.
  - **Solution:** Open your IDE's extension marketplace, search for "Codefly CLI
    Companion", and
