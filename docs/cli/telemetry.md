...
Learn how to enable and setup OpenTelemetry for Codefly CLI.
...
observability framework — Codefly CLI's observability system provides:
...
| `otlpProtocol` | `CODEFLY_TELEMETRY_OTLP_PROTOCOL` | OTLP transport protocol                             | `"grpc"`/`"http"` | `"grpc"`                |
| `outfile`      | `CODEFLY_TELEMETRY_OUTFILE`       | Save telemetry to file (overrides `otlpEndpoint`)   | file path         | -                       |
| `logPrompts`   | `CODEFLY_TELEMETRY_LOG_PROMPTS`   | Include prompts in telemetry logs                   | `true`/`false`    | `true`                  |
| `useCollector` | `CODEFLY_TELEMETRY_USE_COLLECTOR` | Use external OTLP collector (advanced)              | `true`/`false`    | `false`                 |
| `useCliAuth`   | `CODEFLY_TELEMETRY_USE_CLI_AUTH`  | Use CLI credentials for telemetry (GCP target only) | `true`/`false`    | `false`                 |
...
credentials that you use to log in to the Codefly CLI. This is useful in
...
1. Enable telemetry in your `.codefly/settings.json`:
   ...
   ```
2. Run Codefly CLI and send prompts.
...
3. Run Codefly CLI and send prompts.
...
Codefly CLI provides a pre-configured
...
Templates** as "**Codefly CLI Monitoring**".

![Codefly CLI Monitoring Dashboard Overview](/docs/assets/monitoring-dashboard-overview.png)

![Codefly CLI Monitoring Dashboard Metrics](/docs/assets/monitoring-dashboard-metrics.png)

![Codefly CLI Monitoring Dashboard Logs](/docs/assets/monitoring-dashboard-logs.png)

To learn more, check out this blog post:
[Instant insights: Codefly CLI’s new pre-configured monitoring dashboards](https://cloud.google.com/blog/topics/developers-practitioners/instant-insights-codefly-clis-new-pre-configured-monitoring-dashboards/).
...
for Codefly CLI, grouped by category.

#### Sessions

Captures startup configuration and user prompt submissions.

- `codefly_cli.config`: Emitted once at startup with the CLI configuration.
  - **Attributes**:
    - `model` (string)
...
- `codefly_cli.user_prompt`: Emitted when a user submits a prompt.
  - **Attributes**:
    - `prompt_length` (int)
    - `prompt_id` (string)
...
- `codefly_cli.tool_call`: Emitted for each tool (function) call.
  - **Attributes**:
    - `function_name`
    - `function_args`
    - `duration_ms`
...
- `codefly_cli.tool_output_truncated`: Output of a tool call was truncated.
  - **Attributes**:
    - `tool_name` (string)
...
- `codefly_cli.edit_strategy`: Edit strategy chosen.
  - **Attributes**:
    - `strategy` (string)

- `codefly_cli.edit_correction`: Edit correction result.
  - **Attributes**:
...
- `codefly_cli.file_operation`: Emitted for each file operation.
  - **Attributes**:
    - `tool_name` (string)
...
Captures Codefly API requests, responses, and errors.

- `codefly_cli.api_request`: Request sent to Codefly API.
  - **Attributes**:
    - `model` (string)
    - `prompt_id` (string)
    - `request_text` (string, optional)

- `codefly_cli.api_response`: Response received from Codefly API.
  - **Attributes**:
    - `model` (string)
...
- `codefly_cli.api_error`: API request failed.
  - **Attributes**:
    - `model` (string)
    - `error` (string)
    - `error_type` (string)
...
- `codefly_cli.malformed_json_response`: `generateJson` response could not be
  parsed.
  - **Attributes**:
    - `model` (string)
...
- `codefly_cli.slash_command`: A slash command was executed.
  - **Attributes**:
    - `command` (string)
    - `subcommand` (string, optional)
    - `status` ("success" | "error")

- `codefly_cli.slash_command.model`: Model was selected via slash command.
  - **Attributes**:
    - `model_name` (string)

- `codefly_cli.model_routing`: Model router made a decision.
  - **Attributes**:
...
- `codefly_cli.chat_compression`: Chat context was compressed.
  - **Attributes**:
    - `tokens_before` (int)
    - `tokens_after` (int)

- `codefly_cli.chat.invalid_chunk`: Invalid chunk received from a stream.
  - **Attributes**:
    - `error.message` (string, optional)

- `codefly_cli.chat.content_retry`: Retry triggered due to a content error.
  - **Attributes**:
    - `attempt_number` (int)
    - `error_type` (string)
    - `retry_delay_ms` (int)
...
- `codefly_cli.chat.content_retry_failure`: All content retries failed.
  - **Attributes**:
    - `total_attempts` (int)
...
- `codefly_cli.conversation_finished`: Conversation session ended.
  - **Attributes**:
    - `approvalMode` (string)
    - `turnCount` (int)

- `codefly_cli.next_speaker_check`: Next speaker determination.
  - **Attributes**:
    - `prompt_id` (string)
...
- `codefly_cli.flash_fallback`: Switched to a flash model as fallback.
  - **Attributes**:
    - `auth_type` (string)

- `codefly_cli.ripgrep_fallback`: Switched to grep as fallback for file search.
  - **Attributes**:
    - `error` (string, optional)

- `codefly_cli.web_fetch_fallback_attempt`: Attempted web-fetch fallback.
  - **Attributes**:
...
- `codefly_cli.extension_install`: An extension was installed.
  - **Attributes**:
...
- `codefly_cli.extension_uninstall`: An extension was uninstalled.
  - **Attributes**:
    - `extension_name` (string)
    - `status` (string)

- `codefly_cli.extension_enable`: An extension was enabled.
  - **Attributes**:
    - `extension_name` (string)
    - `setting_scope` (string)

- `codefly_cli.extension_disable`: An extension was disabled.
  - **Attributes**:
    - `extension_name` (string)
    - `setting_scope` (string)

- `codefly_cli.extension_update`: An extension was updated.
  - **Attributes**:
    - `extension_name` (string)
    - `extension_version` (string)
...
- `codefly_cli.agent.start`: Agent run started.
  - **Attributes**:
    - `agent_id` (string)
    - `agent_name` (string)

- `codefly_cli.agent.finish`: Agent run finished.
  - **Attributes**:
    - `agent_id` (string)
    - `agent_name` (string)
    - `duration_ms` (int)
...
- `codefly_cli.ide_connection`: IDE companion connection.
  - **Attributes**:
...
- `codefly_cli.session.count` (Counter, Int): Incremented once per CLI startup.

##### Tools

Measures tool usage and latency.

- `codefly_cli.tool.call.count` (Counter, Int): Counts tool calls.
  - **Attributes**:
    - `function_name`
    - `success` (boolean)
...
- `codefly_cli.tool.call.latency` (Histogram, ms): Measures tool call latency.
  - **Attributes**:
    - `function_name`
...
- `codefly_cli.api.request.count` (Counter, Int): Counts all API requests.
  - **Attributes**:
    - `model`
    - `status_code`
    - `error_type` (if applicable)

- `codefly_cli.api.request.latency` (Histogram, ms): Measures API request
  latency.
  - **Attributes**:
    - `model`
...
- `codefly_cli.token.usage` (Counter, Int): Counts tokens used.
  - **Attributes**:
    - `model`
...
- `codefly_cli.file.operation.count` (Counter, Int): Counts file operations.
  - **Attributes**:
    - `operation` ("create", "read", "update")
    - `lines` (Int, optional)
    - `mimetype` (string, optional)
...
- `codefly_cli.lines.changed` (Counter, Int): Number of lines changed (from file
  diffs).
  - **Attributes**:
    - `function_name`
...
- `codefly_cli.chat_compression` (Counter, Int): Counts chat compression
  operations.
  - **Attributes**:
    - `tokens_before` (Int)
    - `tokens_after` (Int)

- `codefly_cli.chat.invalid_chunk.count` (Counter, Int): Counts invalid chunks
  from streams.

- `codefly_cli.chat.content_retry.count` (Counter, Int): Counts retries due to
  content errors.

- `codefly_cli.chat.content_retry_failure.count` (Counter, Int): Counts requests
...
- `codefly_cli.slash_command.model.call_count` (Counter, Int): Counts model
  selections via slash command.
  - **Attributes**:
    - `slash_command.model.model_name` (string)

- `codefly_cli.model_routing.latency` (Histogram, ms): Model routing decision
  latency.
  - **Attributes**:
...
- `codefly_cli.model_routing.failure.count` (Counter, Int): Counts model routing
  failures.
  - **Attributes**:
    - `routing.decision_source` (string)
...
- `codefly_cli.agent.run.count` (Counter, Int): Counts agent runs.
  - **Attributes**:
    - `agent_name` (string)
    - `terminate_reason` (string)

- `codefly_cli.agent.duration` (Histogram, ms): Agent run durations.
  - **Attributes**:
    - `agent_name` (string)

- `codefly_cli.agent.turns` (Histogram, turns): Turns taken per agent run.
  - **Attributes**:
    - `agent_name` (string)
...
- `codefly_cli.plan.execution.count` (Counter, Int): Counts plan executions.
  - **Attributes**:
    - `approval_mode` (string)
...
- `codefly_cli.ui.flicker.count` (Counter, Int): Counts UI frames that flicker
...
- `codefly_cli.startup.duration` (Histogram, ms): CLI startup time by phase.
  - **Attributes**:
    - `phase` (string)
    - `details` (map, optional)

- `codefly_cli.memory.usage` (Histogram, bytes): Memory usage.
  - **Attributes**:
    - `memory_type` ("heap_used", "heap_total", "external", "rss")
    - `component` (string, optional)

- `codefly_cli.cpu.usage` (Histogram, percent): CPU usage percentage.
  - **Attributes**:
    - `component` (string, optional)

- `codefly_cli.tool.queue.depth` (Histogram, count): Number of tools in the
  execution queue.

- `codefly_cli.tool.execution.breakdown` (Histogram, ms): Tool time by phase.
  - **Attributes**:
...
- `codefly_cli.token.efficiency` (Histogram, ratio): Token efficiency metrics.
  - **Attributes**:
    - `model` (string)
    - `metric` (string)
    - `context` (string, optional)

- `codefly_cli.performance.score` (Histogram, score): Composite performance
  score.
  - **Attributes**:
    - `category` (string)
    - `baseline` (number, optional)

- `codefly_cli.performance.regression` (Counter, Int): Regression detection
  events.
  - **Attributes**:
    - `metric` (string)
...
- `codefly_cli.performance.regression.percentage_change` (Histogram, percent):
...
- `codefly_cli.performance.baseline.comparison` (Histogram, percent): Comparison
  to baseline.
  - **Attributes**:
    - `metric` (string)
    - `category` (string)
