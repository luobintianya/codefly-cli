...
2.  **Project commands (local):** Located in
    `<your-project-root>/.codefly/commands/`. These commands are specific to the
...
- A file at `~/.codefly/commands/test.toml` becomes the command `/test`.
- A file at `<project>/.codefly/commands/git/commit.toml` becomes the namespaced
  command `/git:commit`.
...
````toml
# In: <project>/.codefly/commands/git/commit.toml
...
- **Directory listing**: `@{path/to/dir}` is traversed and each file present
  ...
  respects `.gitignore` and `.codeflyignore` if enabled.
...
```toml
# In: <project>/.codefly/commands/review.toml
...
```bash
mkdir -p ~/.codefly/commands/refactor
touch ~/.codefly/commands/refactor/pure.toml
```

**2. Add the content to the file:**

Open `~/.codefly/commands/refactor/pure.toml` in your editor and add the
following content. We are including the optional `description` for best
practice.

```toml
# In: ~/.codefly/commands/refactor/pure.toml
