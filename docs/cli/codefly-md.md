...
for providing instructional context to the Codefly model. You can use these
...
1.  **Global context file:**
    - **Location:** `~/.codefly/CODEFLY.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Project root and ancestor context files:**
    - **Location:** The CLI searches for a `CODEFLY.md` file in your current
...
3.  **Sub-directory context files:**
    - **Location:** The CLI also scans for `CODEFLY.md` files in subdirectories
...
### Example `CODEFLY.md` file

Here is an example of what you can include in a `CODEFLY.md` file at the root of
...
- **`/memory refresh`**: Forces a re-scan and reload of all `CODEFLY.md` files
  from all configured locations.
- **`/memory add <text>`**: Appends your text to your global
  `~/.codefly/CODEFLY.md` file. This lets you add persistent memories on the
  fly.

## Modularize context with imports

You can break down large `CODEFLY.md` files into smaller, more manageable
...
**Example `CODEFLY.md` with imports:**

```markdown
# Main CODEFLY.md file
...
While `CODEFLY.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "CODEFLY.md"]
  }
}
```
