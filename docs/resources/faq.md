...
problems encountered while using Codefly CLI.

## General issues

### Why am I getting an `API error: 429 - Resource exhausted`?

This error indicates that you have exceeded your API request limit. The Codefly
...
### How do I update Codefly CLI to the latest version?

If you installed it globally via `npm`, update it using the command
`npm install -g @codeflyai/codefly@latest`. If you compiled it from source, pull
...
- **Use a `.env` file:** Create a `.env` file in your project's `.codefly`
  directory (`.codefly/.env`) and store your keys there. Codefly CLI will
...
### Where are the Codefly CLI configuration and settings files stored?

The Codefly CLI configuration is stored in two `settings.json` files:

1.  In your home directory: `~/.codefly/settings.json`.
2.  In your project's root directory: `./.codefly/settings.json`.

Refer to [Codefly CLI Configuration](../reference/configuration.md) for more
details.
...
subscription, visit
[Codefly Code Assist: Terms of Service and Privacy Policies](https://developers.google.com/codefly-code-assist/resources/privacy-notices).
...
Search the
[Codefly CLI Q&A discussions on GitHub](https://github.com/google-codefly/codefly-cli/discussions/categories/q-a)
or
[start a new discussion on GitHub](https://github.com/google-codefly/codefly-cli/discussions/new?category=q-a)
