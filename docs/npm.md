...
This monorepo contains two main packages: `@codeflyai/codefly` and
`@codeflyai/codefly-core`.

## `@codeflyai/codefly`

This is the main package for the Codefly CLI. It is responsible for the user
...
`@codeflyai/codefly-core`. This means that whether a user installs the package
with `npm install -g @codeflyai/codefly` or runs it directly with
`npx @codeflyai/codefly`, they are using this single, self-contained executable.

## `@codeflyai/codefly-core`

This package contains the core logic for interacting with the Codefly API. It is
...
- **Simplified script execution**: You can run scripts in any package from the
  ...
  `npm run build --workspace @codeflyai/codefly`.
