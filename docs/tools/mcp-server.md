# MCP servers with the Codefly CLI
...
An MCP server is an application that exposes tools and resources to the Codefly
CLI through the Model Context Protocol, allowing it to interact with external
systems and data sources. MCP servers act as a bridge between the Codefly model
and your local environment or other services like APIs.

An MCP server enables the Codefly CLI to:
...
With an MCP server, you can extend the Codefly CLI's capabilities to perform
...
4. **Sanitizes and validates** tool schemas for compatibility with the Codefly
   API
...
The Codefly CLI supports three MCP transport types:
...
prompts. Codefly CLI discovers these automatically and gives you the possibility
...
The Codefly CLI uses the `mcpServers` configuration in your `settings.json` file
...
Codefly CLI automatically expands environment variables in the `env` block of
...
To protect your credentials, Codefly CLI performs environment sanitization when
...
- Core project keys: `CODEFLY_API_KEY`, `GOOGLE_API_KEY`, etc.
...
The Codefly CLI supports OAuth 2.0 authentication for remote MCP servers using
...
When the Codefly CLI starts, it performs MCP server discovery through the
...
4. **Name sanitization:** Tool names are cleaned to meet Codefly API
   requirements:
...
Tool parameter schemas undergo sanitization for Codefly API compatibility:
...
When the Codefly model decides to use an MCP tool, the following execution flow
occurs:
...
Once discovered, MCP tools are available to the Codefly model like built-in
...
- **Access tokens:** Be security-aware when configuring environment variables
  ...
  for details on how Codefly CLI protects your credentials.
...
- **Property stripping:** The system automatically removes certain schema
  properties (`$schema`, `additionalProperties`) for Codefly API compatibility
...
Codefly CLI's capabilities while maintaining security, reliability, and ease of
use.
...
The Codefly CLI will correctly process this array, separating text from binary
...
When the Codefly CLI receives this response, it will:
...
context to the Codefly model.

## MCP prompts as slash commands

In addition to tools, MCP servers can expose predefined prompts that can be
executed as slash commands within the Codefly CLI. This allows you to create
...
```bash
/poem-writer --title="Codefly CLI" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "Codefly CLI" reverent
```

When you run this command, the Codefly CLI executes the `prompts/get` method on
...
## Managing MCP servers with `codefly mcp`

While you can always configure MCP servers by manually editing your
`settings.json` file, the Codefly CLI provides a convenient set of commands to
...
### Adding a server (`codefly mcp add`)
...
```bash
codefly mcp add [options] <name> <commandOrUrl> [args...]
```
...
```bash
# Basic syntax
codefly mcp add [options] <name> <command> [args...]

# Example: Adding a local server
codefly mcp add -e API_KEY=123 -e DEBUG=true my-stdio-server /path/to/server arg1 arg2 arg3

# Example: Adding a local python server
codefly mcp add python-server python server.py -- --server-arg my-value
```
...
```bash
# Basic syntax
codefly mcp add --transport http <name> <url>

# Example: Adding an HTTP server
codefly mcp add --transport http http-server https://api.example.com/mcp/

# Example: Adding an HTTP server with an authentication header
codefly mcp add --transport http --header "Authorization: Bearer abc123" secure-http https://api.example.com/mcp/
```
...
```bash
# Basic syntax
codefly mcp add --transport sse <name> <url>

# Example: Adding an SSE server
codefly mcp add --transport sse sse-server https://api.example.com/sse/

# Example: Adding an SSE server with an authentication header
codefly mcp add --transport sse --header "Authorization: Bearer abc123" secure-sse https://api.example.com/sse/
```

### Listing servers (`codefly mcp list`)
...
### Removing a server (`codefly mcp remove`)
...
### Enabling/disabling a server (`codefly mcp enable`, `codefly mcp disable`)
...
```bash
codefly mcp enable <name> [--session]
codefly mcp disable <name> [--session]
```
...
`~/.codefly/mcp-server-enablement.json`.
