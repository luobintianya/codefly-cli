...
- **[Memory Import Processor](./memport.md):** Documentation for the modular
  CODEFLY.md import feature using @file.md syntax.
...
- **API key management:** It handles the `CODEFLY_API_KEY` and ensures it's used
  securely when communicating with the Codefly API.
...
To ensure that long conversations don't exceed the token limits of the Codefly
...
[Google AI documentation](https://ai.google.dev/codefly-api/docs/models).

## Model fallback

Codefly CLI includes a model fallback mechanism to ensure that you can continue
...
`CODEFLY.md` files that provide context to the model. It searches for these
...
`refresh` the content of loaded `CODEFLY.md` files.

## Citations

When Codefly finds it is reciting text from a source it appends the citation to
